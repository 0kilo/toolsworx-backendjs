...
function createMcpServer() {
  ...
  const assertSchemaShape = (toolName, schemaOrShape) => {
    if (schemaOrShape && typeof schemaOrShape.safeParse === 'function') {
      console.log(JSON.stringify({ msg: 'MCP inputSchema check', tool: toolName, kind: 'schema' }));
      return;
    }
    for (const [key, value] of Object.entries(schemaOrShape ?? {})) {
      const hasSafeParse = value && typeof value.safeParse === 'function';
      const hasZod = value && typeof value === 'object' && '_zod' in value;
      console.log(
        JSON.stringify({
          msg: 'MCP inputSchema check',
          tool: toolName,
          field: key,
          hasSafeParse,
          hasZod,
      ...
      if (!hasSafeParse) {
        throw new Error(`Invalid inputSchema for ${toolName}: ${key} is not a Zod schema`);
      }
    }
  };
  ...
  // assertSchemaShape('file.convert', fileInputSchema);
  server.registerTool(
    'file.convert',
    {
      description: [
  ...
  // assertSchemaShape('media.convert', mediaInputSchema);
  server.registerTool(
    'media.convert',
    {
      description: [
  ...
  // assertSchemaShape('image.filter', imageFilterInputSchema);
  server.registerTool(
    'image.filter',
    {
      description: [
  ...
  const audioFilterInputSchema = z.object({
    filename: z.string().describe('Original filename with extension'),
    contentBase64: z
      .string()
      .describe('Base64-encoded file content (no data URL prefix)'),
  ...
  // assertSchemaShape('audio.filter', audioFilterInputSchema);
  server.registerTool(
    'audio.filter',
    {
      description: [
      ...
      inputSchema: audioFilterInputSchema
    },
  ...
  for (const [name, tool] of Object.entries(tools)) {
    ...
    console.log(
      JSON.stringify({
        msg: 'MCP tool schema snapshot',
        tool: name,
        hasSchema: !!schema,
        hasZod: schema && typeof schema === 'object' && ('_zod' in schema || '_def' in schema),
        hasSafeParse: schema && typeof schema.safeParse === 'function'
      })
    );
  }
  return server;
