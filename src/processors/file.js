...
async function convertDocument(inputPath, tempDir, targetFormat, inputExt, job) {
  ...
  logger.info({ inputPath, targetFormat, inputExt, outputDir, profileDir }, 'Starting document conversion');
  ...
  await new Promise((resolve, reject) => {
    ...
    logger.info({ libreOfficePath: LIBRE_OFFICE_PATH, args }, 'Spawning LibreOffice');
    ...
    proc.on('close', (code) => {
      clearTimeout(timer);
      if (code === 0) {
        if (stdout) logger.info({ stdout }, 'LibreOffice stdout');
        if (stderr) logger.warn({ stderr }, 'LibreOffice stderr');
        resolve();
      } else {
  ...
  logger.info({ outputFiles: files }, 'LibreOffice output files');
